document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Shared To-Do Rendering Function ---
    window.renderTodoItem = function(todo, { onToggle, onDelete, onEdit, onPriority, onUnarchive } = {}) {
        ...
        todoItem.querySelector('.icon').addEventListener('click', () => onToggle && onToggle(todo));
        // Add click handler for delete
        todoItem.querySelector('.delete-todo-btn').addEventListener('click', (e) => { e.stopPropagation(); onDelete && onDelete(todo); });
        // Add click handler for edit
        if (onEdit) {
            todoItem.querySelector('h3').addEventListener('click', () => onEdit(todoItem, todo));
        }
        // Add click handler for priority (if needed)
        if (onPriority) {
            todoItem.querySelector('.details').addEventListener('contextmenu', (e) => { e.preventDefault(); onPriority(todo); });
        }
        // Add click handler for unarchive (if needed)
        if (onUnarchive) {
            todoItem.addEventListener('dblclick', () => onUnarchive(todo));
        }
        return todoItem;
    };
    ...
    (async () => {
        ...
        const renderTodos = () => {
            ...
            filteredTodos.sort((a, b) => {
                ...
                if (currentFilter === 'archived') return (b.createdAt?.toMillis ? b.createdAt.toMillis() : 0) - (a.createdAt?.toMillis ? a.createdAt.toMillis() : 0);
            ...
            if (filteredTodos.length === 0) {
                ...
                filteredTodos.forEach(todo => {
                    ...
                    const createdDate = todo.createdAt?.toDate ? todo.createdAt.toDate() : (todo.createdAt ? new Date(todo.createdAt) : new Date());
                    itemEl.innerHTML = `
                        ${completionIconHTML}
        ...
        window.addTodoItem = async (text, dueDate, dueTime, priority) => {
            try {
                ...
                if (typeof showToast === 'function') {
                    showToast('To-do item added successfully!', 'success');
                } else {
